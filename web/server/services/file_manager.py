...
class FileManager:
    def __init__(self, settings: Settings):
        ...
        self._file_cache: Dict[str, str] = {}  # Cache file_id -> file_path mappings
    ...
    def get_file_path(self, file_id: str) -> str:
        """Get file path by file ID (with caching)"""
        # Check cache first
        if file_id in self._file_cache:
            cached_path = self._file_cache[file_id]
        ...
        for ext in self.settings.allowed_file_types:
            ...
            if os.path.exists(potential_path):
                self._file_cache[file_id] = potential_path
