...
class EDAEngine:
    ...
    async def _analyze_correlations(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Analyze correlations between numeric variables (optimized)"""
        ...
        # Convert correlation matrix to native Python types (vectorized)
        corr_dict = correlation_matrix.to_dict()

        # Find strong correlations (optimized - only check upper triangle)
        strong_correlations = []
        columns_list = correlation_matrix.columns.tolist()
            ...
            for j in range(i + 1, len(columns_list)):
                ...
                if abs(corr_value) > 0.5:  # Threshold for strong correlation
                    strong_correlations.append({
                        'variable1': columns_list[i],
                        'variable2': columns_list[j],
    ...
    def _prepare_heatmap_data(self, correlation_matrix: pd.DataFrame) -> List[Dict]:
        """Prepare correlation matrix data for heatmap visualization (optimized)"""
